...
const NotificationsDropdown = () => {
    ...
    return (
        <div className="relative">
            ...
            {isOpen && (
                <div className="absolute right-0 mt-3 w-80 bg-white border border-gray-200 rounded-md shadow-lg z-50">
                    <div className="p-3 font-bold text-brand-dark bg-brand-light rounded-t-md flex justify-between items-center">
                        <span>Notifications</span>
                        {isAdmin && notifications.length > 0 && (
                            <button
                                onClick={markAllAsRead}
                                className="text-xs text-gray-700 hover:text-brand-dark"
                            >
                                Mark all as read
                            </button>
                        )}
                            ...
                            {notifications.map((notification) => (
                                <li
                                    ...
                                    className={`p-3 border-b text-sm flex justify-between items-center ${
                                        notification.read ? "text-gray-500" : "font-semibold text-gray-900 bg-gray-50"
