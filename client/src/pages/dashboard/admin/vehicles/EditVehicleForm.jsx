...
const EditVehicleForm = () => {
    ...
    const handleInputChange = (e) => {
        ...
        if (trimmedValue !== "") { 
            if (name === "vehicleInspection") {
                ...
                if (errorInPast) {
                    setVehicleErrors((prev) => ({ ...prev, vehicleInspection: errorInPast }));
                    return;
                }
    ...
    return (
        <div className="max-w-2xl mx-auto mt-6 bg-white shadow-lg rounded-xl p-6 relative">
            ...
            {vehicle && (
                <div className="mb-4 p-3 rounded-xl bg-purple-50 text-sm grid grid-cols-2 gap-2">
                    <p><strong>Brand:</strong> {vehicle.brand}</p>
                    <p><strong>Model:</strong> {vehicle.model}</p>
                    <p><strong>License Plate:</strong> {vehicle.licensePlate}</p>
                    <p><strong>Type:</strong> {vehicle.type}</p>
                    <p><strong>Capacity:</strong> {vehicle.capacity} seats</p>
                    <p><strong>Inspection Date:</strong> {formatDate(vehicle.vehicleInspection)}</p>
                    <p className="col-span-2"><strong>Status:</strong> {vehicle.status}</p>
                </div>
            )}
            <p className="text-center text-gray-600 mb-4 text-sm">Enter new values for the fields you want to update.</p>
            <form onSubmit={handleUpdateVehicle} className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <FormInputField 
                    label="Brand" 
                    name="brand" 
