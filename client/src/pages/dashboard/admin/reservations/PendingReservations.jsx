  ...
  return (
    <div className="p-6 w-full mx-auto">
      ...
      reapprovalReservations.length === 0 ? (
        ...
        <>
          {reapprovalReservations.length > 0 && (
            <div className="p-4 border border-purple-200 rounded-base bg-gray-200 mb-8">
              <h3 className="text-lg font-bold text-brand-darker mb-3">
                Modified Reservations
              </h3>
              <div className="text-sm bg-white p-3 rounded-base mb-4 border-l-4 border-brand-darker">
                <p className="text-brand-dark">
                  <strong>Important:</strong> Some reservations have been
                  modified by users and require your review. If no action is
                  taken, the original details will remain active.
                </p>
              </div>
              <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {reapprovalReservations.map((res) => (
                  <li
                    key={res._id}
                    className="relative bg-white shadow-md rounded-base p-4 flex flex-col space-y-3 hover:shadow-lg border border-brand-lighter transition-all duration-300"
                  >
                    ...
                    <Link
                      ...
                      className="mt-3 text-sm text-brand-dark bg-purple-100 hover:bg-purple-200 rounded-base py-2 text-center shadow-sm transition-all"
                    >
                      Review Changes
                    </Link>
                  </li>
          ...
          {pendingReservations.length > 0 && (
            <div className="p-4 border border-gray-200 rounded-base bg-white">
              <h3 className="text-lg font-bold text-brand-darker mb-3">
                Pending Approvals
              </h3>
              <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
