...
const VehicleDragAndDropList = ({ groupedVehicles, onDragEnd }) => {
    return (
        <Droppable droppableId="vehiclesList" direction="vertical">
            {(provided) => (
                <div
                    className="p-4 bg-gray-50 rounded overflow-y-auto max-h-64 shadow-sm"
                    {...provided.droppableProps}
                    ref={provided.innerRef}
                >
                    {Object.entries(groupedVehicles).map(([letter, groupedVehicles]) => (
                        <div key={letter} className="mb-4">
                            <div className="text-sm font-bold text-gray-800 mb-2">{letter}</div>
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                {groupedVehicles.map((veh, index) => (
                                    <Draggable
                                        ...
                                        draggableId={veh._id}
                                        index={index}
                                        isDragDisabled={veh.status === "occupied" || veh.status === "service"}
                                    >
                                        {(provided) => (
                                            <div
                                                ref={provided.innerRef}
                                                ...
                                                className={`p-2 rounded shadow-sm text-sm cursor-grab ${
                                            ...
                                            >
                                                ...
                                                {(veh.status === "occupied" || veh.status === "service") && (
                                                    <p className="text-error text-md font-bold capitalize">{veh.status}</p>
                                                )}
                                            </div>
                                        )}
                                    </Draggable>
