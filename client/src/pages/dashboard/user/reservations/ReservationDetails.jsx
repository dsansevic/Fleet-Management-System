...
const ReservationDetails = () => {
  ...
  return (
    <div className="p-6 lg:px-20 sm:w-screen max-w-3xl mx-auto my-auto h-full">
      <div className="bg-white shadow-md border rounded-base p-6 space-y-4 relative">
        <button
          onClick={() => navigate(-1)}
        ...
        <div className="text-center">
          ...
          <div
            className={`inline-flex items-center px-3 py-1 mt-2 text-md font-medium ${statusInfo.color} `}
          >
            <span className="ml-2">{statusInfo.message}</span>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-6 text-sm rounded-base text-gray-700 bg-purple-50 p-4 pl-10">
          <p>
          ...
          {reservation.newEndTime && (
            <p>
              <span className="font-bold">Proposed ending:</span>{" "}
          ...
          {reservation.vehicle && (
            <p className="col-span-2">
              <span className="font-bold">Assigned vehicle:</span>{" "}
              {reservation.vehicle.brand}, {reservation.vehicle.model},{" "}
              {reservation.vehicle.type}, {reservation.vehicle.licensePlate},
              seat number: {reservation.vehicle.capacity}
            </p>
          )}
        </div>
        ...
        {isEditing && canEditOrCancel ? (
          ...
          <>
            <div className="flex justify-between">
              <button
                className="bg-white text-brand-base px-4 py-2 rounded-base shadow shadow-gray-300 hover:bg-gray-50 flex items-center space-x-2"
              ...
              <button
                className="bg-white text-gray-500 px-4 py-2 rounded-base shadow shadow-gray-300 hover:bg-gray-50 flex items-center space-x-2"
      ...
      <Modal
        ...
        content={
          <p>
            Keep in mind you won't be able to reactivate this reservation once
            canceled. You will have to create a new one.
          </p>
        }
        actions={[
          {
            label: "No, go back",
            className: "bg-gray-300 text-gray-800 px-4 py-2 rounded-base",
            onClick: () => setShowCancelConfirm(false),
          },
          {
            label: "Yes, cancel",
            className:
              "bg-red-500 text-white px-4 py-2 rounded-base shadow hover:bg-red-600",
